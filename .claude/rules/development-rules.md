...
## General
- **Communication Language**: Always respond to the user in Vietnamese. However, everything related to the project (documentation, code comments, commit messages, etc.) MUST be written in English.
- **English Grammar Correction**: Before answering an English question in Vietnamese, always provide a corrected version of the user's question first in the format: "Correct: ......" to support the user's English learning.
- **CLI Explanation**: Before executing any CLI command, provide a brief explanation of what the command does and why it is being run.
...
## Debugging Rules
- **Rule 5: Find the root cause when fixing a bug**: Always try to find and fix the root cause instead of using a workaround or fallback.
- **Rule 6: Always revert if it doesn't fix the issue**: When you try to fix a bug, if it doesn't work, always revert the changes before trying new ways to fix it.
